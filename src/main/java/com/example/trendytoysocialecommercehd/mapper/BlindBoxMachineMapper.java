...
public interface BlindBoxMachineMapper extends BaseMapper<BlindBoxMachine> {
     ...
     */
    @Select("SELECT m.*, s.shop_name, ss.sale_title as sale_series_name " +
            "FROM blind_box_machine m " +
            "LEFT JOIN shop s ON m.shop_id = s.shop_id " +
            "LEFT JOIN sale_series ss ON m.sale_series_id = ss.sale_series_id " +
     ...
     */
    @Select("<script>" +
            "SELECT m.*, s.shop_name, ss.sale_title as sale_series_name " +
            "FROM blind_box_machine m " +
            "LEFT JOIN shop s ON m.shop_id = s.shop_id " +
            "LEFT JOIN sale_series ss ON m.sale_series_id = ss.sale_series_id " +
            ...
            "       OR ss.sale_title LIKE CONCAT('%', #{keyword}, '%')) " +
            "</if>" +
